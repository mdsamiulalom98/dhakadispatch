...
<div class="page-header">
	<div class="row">
		<div class="col-sm-6"><h6><strong>Payment</strong></h6></div>
		<div class="col-sm-6">
			<div class="payment-btns text-end">
				<ul>
					...
					<li><a href="{{route('merchant.parcel.payable')}}" class="btn btn-success">Payable Parcel</a></li>
				</ul>
			</div>
		</div>
	</div>
...
<div class="page-content">
	<div class="row">
		...
		<div class="col-sm-3">
			<div class="payable-box">
				<p>Payable Amount</p>
				<h4>৳ {{$paycod - ($delivery_charge + $cod_charge)}}</h4>
			</div>
		</div>
		<!-- col end -->
	</div>
			...
			<form  class="filter-form">
                <div class="form-group">
                    <select class="form-control select2" name="filter" onchange="this.form.submit()">
					    ...
					    <option value="last-month" {{ request('filter') == 'last-month' ? 'selected' : '' }}>Last Month</option>
					    <option value="year" {{ request('filter') == 'year' ? 'selected' : '' }}>This Year</option>
					    <option value="last-year" {{ request('filter') == 'last-year' ? 'selected' : '' }}>Last Year</option>
					</select>
                </div>
	            <!--col-sm-3-->
			</form>
			...
			<table class="table table-responsive">
				...
				<tbody>
					...
					<tr>
						...
						<td><a href="{{route('merchant.payment.invoice',$value->invoice_id)}}">{{$value->invoice_id}} <p class="mobile-show mt-2">Date: {{$value->created_at->format('d m, Y h:m A')}}</p></a></td>
						...
						<td class="desktop-show"><a href="{{route('merchant.payment.invoice',$value->invoice_id)}}" class="btn-view">View</a></td>
					</tr>
					@endforeach
				</tbody>
	  ...
	  <div class="modal-dialog">
	    <div class="modal-content">
	      ...
	      <div class="modal-body">
	        <form action="{{ route('merchant.payment.request') }}" method="POST"
                    ...
                    <div class="col-sm-12 mb-3">
                        <div class="form-group">
                            ...
                            <select class="form-control" name="payment_method" id="payment_method">
                                ...
                                <option value="bank" data-method="{{$merchatnpay->account_number ?? ''}}" {{Auth::guard('merchant')->user()->default_method == 'bank'? 'selected':''}}>Bank</option>
                                <option value="bkash" data-method="{{$merchatnpay->bkash ?? ''}}" {{Auth::guard('merchant')->user()->default_method == 'bkash'? 'selected':''}}>Bkash</option>
                                <option value="nagad" data-method="{{$merchatnpay->nagad ?? ''}}" {{Auth::guard('merchant')->user()->default_method == 'nagad'? 'selected':''}}>Nagad</option>
                                <option value="rocket" data-method="{{$merchatnpay->rocket ?? ''}}" {{Auth::guard('merchant')->user()->default_method == 'rocket'? 'selected':''}}>Rocket</option>
                                <option value="cash" {{Auth::guard('merchant')->user()->default_method == 'cash'? 'selected':''}}>Cash</option>
                            </select>
                        </div>
