...
@section('content')
	<div class="page-header">
		<div class="parcle-header">	
			...
			<form class="mobile-show">
				<select onchange="window.location.href=this.value" class="form-select">
				    <option value="{{route('merchant.parcel.index','all')}}">All</option>
				    @foreach($parcelstatus as $status)
				    <option value="{{route('merchant.parcel.index',$status->slug)}}" {{$type == $status->name ? 'selected' : ''}}>{{$status->name}}</option>
				    @endforeach
				</select>
			</form>
		</div>	
		<div class="manage-btns">
			<ul>
				<li><a href="{{route('merchant.parcel.index','all')}}"class="bg-info {{$type == 'All' ? 'active' :''}}">All</a></li>
				@foreach($parcelstatus as $status)
				<li><a href="{{route('merchant.parcel.index',$status->slug)}}" class="bg-info {{$type == $status->name ? 'active' :''}}">{{$status->name}}</a></li>
				@endforeach
			</ul>
		</div>
	</div>
				...
				<table class="table table-responsive parcel-table">
					...
					<tbody>
						...
						<tr>
							...
							<td class="desktop-show"><a href="{{route('merchant.parcel.view',$value->parcel_id)}}" class="btn-view">View</a></td>
						</tr>
						@endforeach
					</tbody>
				</table>
