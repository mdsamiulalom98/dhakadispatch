		...
		<div class="col-sm-6">
			<div class="payment-btns text-end">
				<ul>
					...
					<li><a href="{{route('rider.parcel.payable')}}" class="btn btn-success">Payable Parcel</a></li>
				</ul>
			</div>
		</div>
	</div>
			...
			<table class="table table-responsive">
				...
				<tbody>
					...
					<tr>
						...
						<td><a href="{{route('rider.payment.invoice',$value->invoice_id)}}">{{$value->invoice_id}} <p class="mobile-show mt-2">Date: {{$value->created_at->format('d m, Y h:m A')}}</p></a></td>
						...
						<td class="desktop-show"><a href="{{route('rider.payment.invoice',$value->invoice_id)}}" class="btn-view">View</a></td>
					</tr>
					@endforeach
				</tbody>
				...
				<tfoot class="payment-footer desktop_none">
					<tr>
						<td><strong>Total</strong></td>
						<td colspan="3">
	  ...
	  <div class="modal-dialog">
	    <div class="modal-content">
	      ...
	      <div class="modal-body">
	        <form action="{{ route('rider.payment.request') }}" method="POST"
                    ...
                    <div class="col-sm-12 mb-3">
                        <div class="form-group">
                            ...
                            <select class="form-control" name="payment_method" id="payment_method">
                                ...
                                <option value="bank" data-method="{{$riderpay->account_number ?? ''}}" {{Auth::guard('rider')->user()->default_method == 'bank'? 'selected':''}}>Bank</option>
                                <option value="bkash" data-method="{{$riderpay->bkash ?? ''}}" {{Auth::guard('rider')->user()->default_method == 'bkash'? 'selected':''}}>Bkash</option>
                                <option value="nagad" data-method="{{$riderpay->nagad ?? ''}}" {{Auth::guard('rider')->user()->default_method == 'nagad'? 'selected':''}}>Nagad</option>
                                <option value="rocket" data-method="{{$riderpay->rocket ?? ''}}" {{Auth::guard('rider')->user()->default_method == 'rocket'? 'selected':''}}>Rocket</option>
                                <option value="cash" {{Auth::guard('rider')->user()->default_method == 'cash'? 'selected':''}}>Cash</option>
                            </select>
                        </div>
