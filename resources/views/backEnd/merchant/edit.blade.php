...
<div class="container-fluid">
        ...
        <div class="col-12">
            <div class="page-title-box">
                <div class="page-title-right">
                    <a href="{{route('merchants.index')}}" class="btn btn-primary rounded-pill">Manage</a>
                </div>
                <h4 class="page-title">Merchant Edit</h4>
            </div>
        </div>
    </div>       
    <!-- end page title --> 
   <div class="row">
    <div class="col-lg-12">
        <div class="card">
            <div class="card-body">
                <form action="{{route('merchants.update')}}" method="POST" class=row data-parsley-validate=""  enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="col-sm-6 mb-3">
                        ...
                        <div class="form-group">
                            <select class="form-control select2 district" name="district_id" id="district" required>
                                <option value="">Select..</option>
                                @foreach ($districts as $key => $value)
                                    <option value="{{ $value->id}}" {{$value->id == $edit_data->district_id ? 'selected' : ''}}>
                                        {{ $value->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-sm-6 mb-3">
                        <div class="form-group">
                            ...
                            <select class="form-control select2 area" name="area_id" id="area" required>
                                <option value="">Select..</option>
                                 @foreach ($areas as $key => $value)
                                    <option value="{{ $value->id}}" {{$value->id == $edit_data->area_id ? 'selected' : ''}}>
                                        {{ $value->name }}
                                    </option>
                                @endforeach
                            </select>
