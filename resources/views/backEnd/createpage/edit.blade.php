        ...
        <div class="card">
            <div class="card-body">
                <form action="{{route('pages.update')}}" method="POST" class=row data-parsley-validate=""  enctype="multipart/form-data">
                    ...
                    <!-- <div class="col-sm-6">
                        <div class="form-group mb-3">
                            <label for="section" class="form-label">Page Section *</label>
                            <select type="text" class="form-control @error('section') is-invalid @enderror" name="section" value="{{ old('section') }}"  id="section" required="">
                                <option value="">Select..</option>
                                <option value="1" @if($edit_data->section==1) selected @endif>User Guidline</option>
                                <option value="2" @if($edit_data->section==2) selected @endif>Important Link</option>
                            </select>
                            @error('section')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
